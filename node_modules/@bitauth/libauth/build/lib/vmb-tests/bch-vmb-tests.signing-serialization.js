...
const verifyAlgorithm = algorithms.map((algorithm) => [
...
const changeScenario = (testDefinitions, appendDescription, newScenario) => testDefinitions.map(([unlockingScript, redeemOrLockingScript, testDescription, testSetOverrideLabels]) => [unlockingScript, redeemOrLockingScript, `${testDescription}${appendDescription}`, testSetOverrideLabels, newScenario]);
...
const verifyAlgorithmWithTokensInMultipleInputsAndOutputs = algorithms.map((algorithm) => [
    `<signing_serialization.full_${algorithm}> <key1.schnorr_signature.${algorithm}>`,
    '<key1.public_key> OP_2DUP OP_CHECKSIGVERIFY OP_SWAP OP_SIZE <1> OP_SUB OP_SPLIT OP_DROP OP_ROT OP_SHA256 OP_ROT OP_CHECKDATASIG',
